...
export default function Sidebar({ activeFilter, onFilterChange, onExport, onShare }) {
    ...
    <aside style={{
      ...
      <div style={{ borderTop: `1px solid ${LINE}`, paddingTop: 20 }}>
        ...
        <button className="action-btn" onClick={onExport}>
          <Share size={12} style={{ opacity: 0.5 }} />
          Export Dossier
        </button>
        ...
        <button className="nav-btn" onClick={() => { if (window.confirm('Confirm secure termination of current session?')) { window.location.href = '/auth'; } }} style={{ color: '#f87171' }}>
          <XCircle size={16} />
          Secure Logout
        </button>
      </div>
