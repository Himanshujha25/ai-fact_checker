  ...
  return (
    <div style={{ background: '#08080E', minHeight: 'calc(100vh - 64px)', color: TEXT, fontFamily: "'DM Sans', system-ui, sans-serif", display: 'flex', paddingBottom: 64 }}>
      <style>{`
        ...
        /* ── All card styles exactly as original ── */
        .arch-pipeline-card {
        ...
        .arch-model-card.active { background: ${GOLD2}; border-color: rgba(201,168,76,0.3); }
        ...
        @media (max-width: 768px) {
          ...
          /* Connector arrows don't make sense in a wrapped grid */
      ...
      <main className="arch-main" style={{ flex: 1, maxWidth: 1240, margin: '0 auto', padding: '6px 48px 80px', overflowY: 'auto' }}>
        ...
        <section style={{ marginBottom: 52 }}>
          ...
          {/* className="arch-pipe-grid" lets mobile override the inline 5-col grid */}
          <div className="arch-pipe-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {PIPELINE.map((step, i) => (
              ...
              >
                <span style={{ position: 'absolute', top: 14, right: 16, fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 52, fontWeight: 400, lineHeight: 1, color: step.id === '05' ? `rgba(201,168,76,0.12)` : 'rgba(255,255,255,0.04)', userSelect: 'none', pointerEvents: 'none' }}>
                  {step.id}
                </span>
                ...
                {/* className="arch-pipe-arrow" hides on mobile */}
                {i < PIPELINE.length - 1 && (
        ...
        {/* className="arch-bottom-grid" stacks to 1col on tablet */}
        <div className="arch-bottom-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 24 }}>
          ...
          <section>
            ...
            {/* className="arch-integrity-grid" collapses to 1col on mobile */}
          ...
          <section>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {MODELS.map((m, i) => (
                <motion.div key={i} className={`arch-model-card ${m.active ? 'active' : ''} ${m.legacy ? 'legacy' : ''}`}
                  initial={{ opacity: 0, x: 10 }} animate={{ opacity: m.legacy ? 0.38 : 1, x: 0 }}
                  transition={{ delay: 0.3 + i * 0.1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: m.active ? GOLD : m.legacy ? DIM : MUTED }}>
                      {m.status}
                    </span>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.65 }}
                style={{ marginTop: 6, background: GOLD2, border: `1px solid rgba(201,168,76,0.22)`, borderRadius: 12, padding: '16px 18px' }}>
                ...
                <p style={{ fontSize: 11, color: MUTED, lineHeight: 1.65 }}>
                  Target{' '}
                  <code style={{ fontFamily: "'DM Mono', monospace", color: GOLD, fontSize: 11 }}>/v1/stable/verify</code>
